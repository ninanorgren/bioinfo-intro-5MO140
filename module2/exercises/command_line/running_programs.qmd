...
#### **The module system on Kebnekaise**

On HPC cluster most users don't have the correct permissions to install programs on the cluster, as this would be very difficult to manage. Kebnekaise instead uses a module system, where several different softwares are installed into. By loading a specific module, you will load the program into your session. So instead of installing the program yourself, you can just load the correct module and it will be there.

You can search for which programs are installed on Kebnekaise by typing:

```
module spider <your search term>
```

You would then load a program by:

```
module load <program + version>
```

Loaded programs are only loaded in your session, so if you close down the session you have to load them again when returning.

<br>

#### **Load modules for bcftools** 

bcftools is a very useful command line program to manipulate and filter VCF files. It can run directly on gzipped files that are indexed with tabix, another program that creates indexes of VCF files, which allows for faster access. Start by loading bcftools and tabix using the module system. The tabix command is part of the HTSlib program.

<details>
  <summary>See code:</summary>
````{bash}
module spider bcftools       # search for bcftools
module spider BCFtools/1.19  # to see the requirements for the latest version
module load GCC/13.2.0       # requirement for bcftools
module load BCFtools/1.19    # load the bcftools program

#repeat above for htslib
module load HTSlib/1.19.1    # loads the tabix command
````
</details>

Once the programs are loaded, verify that everything seems to be installed correctly by running:
...
Let's start using bcftools to first filter out our extra chromosomes that we are not interested in. For this we will use bcftools view, on an indexed vcf file. Use tabix to index the file. Try reading the documentation and figure out how we can filter to only keep chromosomes 1-22, there are a few different ways. Remember to keep your rawdata separate from your filtered data.
